...
Deno.test('Nuewframe OAuth CLI - main module file exists', async () => {
...
Deno.test('Nuewframe OAuth CLI - main command metadata is set', () => {
...
Deno.test('Nuewframe OAuth CLI - top-level commands are registered', () => {
...
Deno.test('Nuewframe OAuth CLI - command exports are defined', () => {
...
Deno.test('Nuewframe OAuth CLI - command descriptions are non-empty', () => {
  const commands = [
    loginCommand,
    serviceCommand,
    configCommand,
...
Deno.test('Nuewframe OAuth CLI - login command registers expected subcommands', () => {
...
Deno.test('Nuewframe OAuth CLI - service command registers token subcommand', () => {
  const cmd = serviceCommand.getCommand('token');
  assertExists(cmd);
  assertEquals(cmd.getName(), 'token');
});

Deno.test('Nuewframe OAuth CLI - token command registers expected subcommands', () => {
